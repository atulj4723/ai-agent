...
// === Tool Functions ===
const create_File = ({ fileName, folderPath = "default", content }) => {
    const dirPath = path.join("./generated-content", folderPath);
    if (!fs.existsSync(dirPath)) fs.mkdirSync(dirPath, { recursive: true });
    fs.writeFileSync(path.join(dirPath, fileName), content);
    return `✅ Created ${fileName} in ${folderPath}`;
};

const read_File = ({ fileName, folderPath = "default" }) => {
    const filePath = path.join("./generated-content", folderPath, fileName);
    return fs.existsSync(filePath)
        ? fs.readFileSync(filePath, "utf-8")
        : `❌ ${fileName} not found in ${folderPath}`;
};

const delete_File = ({ fileName, folderPath = "default" }) => {
    const filePath = path.join("./generated-content", folderPath, fileName);
    if (fs.existsSync(filePath)) {
        fs.unlinkSync(filePath);
        return `🗑️ Deleted ${filePath}`;
    }
    return `❌ ${fileName} not found in ${folderPath}`;
};

const list_Files = ({ folderPath = "default" }) => {
    const folderPath = path.join("./generated-content", folderPath);
    return fs.existsSync(folderPath)
        ? fs.readdirSync(folderPath).join(", ")
        : `❌ Folder '${folderPath}' not found`;
};

const append_File = ({ fileName, folderPath = "default", content }) => {
    const filePath = path.join("./generated-content", folderPath, fileName);
    if (fs.existsSync(filePath)) {
        fs.appendFileSync(filePath, content);

        return `➕ Appended to ${fileName} in ${folderPath}`;
    }
    return `❌ ${fileName} not found in ${folderPath}`;
};
